...
def Num(P, F, mf, t):
    if F == 2:
        y = P*(sl2*R2(mf, mf)*bmfl[abs(mf)]+sp2*R2(mf, mf+1)*bmfp[mf]+sn2*R2(mf, mf-1)*bmfn[mf])
        N = np.zeros(len(t))
        for j in range(1, len(t)):
            N[j] = N[j-1] + (t[j] - t[j-1]) * (y[j] + y[j-1]) / 2
        return N
    elif F == 1:
        y = P*(sl1*R1(mf, mf)*amfl[abs(mf)]+sp1*R1(mf, mf+1)*amfp[mf]+sn1*R1(mf, mf-1)*amfn[mf])
        N = np.zeros(len(t))
        for j in range(1, len(t)):
            N[j] = N[j-1] + (t[j] - t[j-1]) * (y[j] + y[j-1]) / 2
        return N
...
sp2, sl2, sn2 = 0.018, 1 - 0.018*2, 0.018
...
P2[0, 1] = b12*b22*R2(1,2)
P2[1, 1] = -b12*(b22+a12)*R2(1, 2)  
P2[5, 1] =  b12*a12*R2(1, 2)            

P2[0, 2] =  b01*b12*R2(0, 1)
P2[1, 2] =  b01*b11*R2(0, 1)
P2[2, 2] =  -b01*(b11+b12+a01+a11)*R2(0, 1)
P2[5, 2] =  b01*a11*R2(0, 1)
P2[6, 2] =  b01*a01*R2(0, 1)

P2[2, 3] =  b01*b01*R2(-1, 0)
P2[3, 3] =  -b01*(b01+a10+a00+a10)*R2(-1, 0)
P2[5, 3] =  b01*a10*R2(-1, 0)
P2[6, 3] =  b01*a00*R2(-1, 0)
P2[7, 3] =  b01*a10*R2(-1, 0)

P2[1, 4] =  b12*b01*R2(-2, -1)
P2[3, 4] =  b12*b11*R2(-2, -1)
P2[4, 4] =  -b12*(b11+b01+a11+a01)*R2(-2, -1)
P2[6, 4] =  b12*a01*R2(-2, -1)
P2[7, 4] =  b12*a11*R2(-2, -1)

# F = 1
P1 = np.zeros((8,8))
P1[0, 5] =  a12*b22*R1(1, 2)
P1[1, 5] =  a12*b12*R1(1, 2)
P1[5, 5] =  -a12*(b22+b12)*R1(1, 2)

P1[0, 6] =  a01*b12*R1(0, 1)
P1[1, 6] =  a01*b11*R1(0, 1)
P1[2, 6] =  a01*b01*R1(0, 1)
P1[5, 6] =  a01*a11*R1(0, 1)
P1[6, 6] =  -a01*(b01+b11+b12+a11)*R1(0, 1)

P1[1, 7] =  a10*b01*R1(-1, 0)
P1[3, 7] =  a10*b01*R1(-1, 0)
P1[5, 7] =  a10*a10*R1(-1, 0)
P1[6, 7] =  a10*a00*R1(-1, 0)
P1[7, 7] =  -a10*(b01+b01+a10+a00)*R1(-1, 0)
...
M = sp1*P1+sp2*P2+sl1*L1+sl2*L2+sn1*N1 + sn2*N2
